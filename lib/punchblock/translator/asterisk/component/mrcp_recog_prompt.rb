      ...
      module Component
        module MRCPRecogPrompt
          ...
          def validate
            ...
            raise OptionError, "A recognition-timeout value must be a positive integer." if @recognition_timeout < 1
            raise OptionError, "A speech-complete-timeout value must be a positive integer." if @speech_complete_timeout < -1
            raise OptionError, "A speech-incomplete-timeout value must be -1 or a positive integer." if @speech_incomplete_timeout < -1
          ...
          def unimrcp_app_options
            {uer: 1, b: (@component_node.barge_in == false ? 0 : 1)}.tap do |opts|
              ...
              opts[:sint] = input_node.speech_incomplete_timeout if @speech_incomplete_timeout > -1
              yield opts
            end
          end
